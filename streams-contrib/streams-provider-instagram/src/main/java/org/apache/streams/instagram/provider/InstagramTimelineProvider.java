...
//            loadBatch(screenNameBatches.next());
    ...
    private void loadBatch(Long[] ids) {
        ...
        Instagram client = getInstagramClient();
        int keepTrying = 0;
        ...
        //while (keepTrying < 1)
        {
            try
            {
                ...
                UserInfo[] users = new UserInfo[ids.length];
                for(int i = 0; i < ids.length; i++)
                    //toQuery[i] = ids[i];
                    users[i] = client.getUserInfo(i);

                for (UserInfo userInfo : users) {
                //for (User tStat : client.lookupUsers(toQuery)) {

                    InstagramTimelineProviderTask providerTask = new InstagramTimelineProviderTask(this, client, userInfo.getData().getId());
                ...
                keepTrying += InstagramErrorHandler.handleInstagramError(client, instagramException);
            }
            catch(Exception e) {
                keepTrying += InstagramErrorHandler.handleInstagramError(client, e);
            }
        }
    }
        ...
        while (keepTrying < 1)
        {
            try
            {
                for (String strId : ids) {
                    ...
                    //MediaFeed userFeed = client.getUserFeeds();
                    UserInfo userInfo = client.getUserInfo(id);
    ...
    public class InstagramTimelineProviderTask implements Runnable {
        ...
        public void run() {
            ...
            List<MediaFeedData> mediaFeeds = null;
...
//                    try
//                    {
//                        mediaFeed = client.getUserFeeds();
//                        mediaFeeds = mediaFeed.getData();
//
//                        for (MediaFeedData mFeed : mediaFeeds)
//                        {
//                            String json = InstagramObjectFactory.getRawJSON(mFeed);
//
//                            try {
...
//                }
//            }
//            while (provider.shouldContinuePulling(mediaFeeds));
